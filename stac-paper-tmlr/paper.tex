...
% ESTAC is important for comparison.
% Risky mass point ablation is very important.
...
Although using pessimistic critic targets for critic and actor training is not a new idea \citep{moskovitz2021tactical, kuznetsov2020controlling, chen2021randomized}, the question of how to determine pessimism $\beta$ remains. 
...
%Distributional methods perform optimistic/pessimistic update to scale risk seeking/averse learning. In general, conditional value at risk (CVaR) measures of value distribution is used for this purpose \citep{dabney2018implicit, tang2019worst, yang2021wcsac, kim2022efficient, ma2025dsac}, while using standard deviation is also practical \citep{ma2025dsac}. 
...
In STAC, pessimism is modeled as a fixed parameter, unlike the work of \citep{moskovitz2021tactical} and \citep{cetin2023learning}. Introducing such a mechanism would increase complexity of the algorithm, and it is observed that a properly selected pessimism rate is enough to obtain sufficient performance, as shown in the experiments. 
...
\paragraph{Sample efficiency} As seen from Figure \ref{fig:main_comparison} and Table \ref{tab:env_auc_results}, STAC outperforms other algorithms on many environments in terms of sample efficiency (see \verb|BipedalWalkerHardcore-v3|, \verb|HalfCheetah-v4|, \verb|Hopper-v4|, \verb|Swimmer-v4| \verb|Walker-v4|). Although ESTAC and SAC performs better on some environments, STAC still has a competitive performance, no convergence issue, and achieves this by using single critic while others use double. STAC has different components compared to other algorithms: pessimism is a must for convergence, while actor dropout is optional for performance improvement. However, critic dropout is necessary for some environments. Details will be discussed in the next section. 
...
For each $\beta$ value, STAC is run on this environment with a same single seed. In Figure \ref{fig:pointmass_heatmap}, for varying $\beta$ values, position densities of the trained agents are demonstrated. The bigger green circles represent danger zone boundaries, whereas quarter green circles on the lower left represent the target points. For this demonstration, 200 evaluation steps are run with random initial points. Clearly, with higher $\beta$ values, the agent prefers to go around the circle as opposed to agents trained with lower $\beta$.  

As expected, STAC not only mitigates overestimation but also ensures risk-averse learning with pessimism on temporal uncertainty. 
