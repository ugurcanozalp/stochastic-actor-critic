...
Most actor-critic methods use epistemic uncertainty for pessimism to mitigate critic overestimation, which hinders exploration of state-action space \citep{moskovitz2021tactical, chen2021randomized}, while some works propose to use both epistemic and aleatoric uncertainty for this purpose \citep{kuznetsov2020controlling}. 

Modeling one step aleatoric return distribution is first proposed by \citet{achab2023one} and shown to have convergence guarantees. Our contribution is to use this paradigm for overestimation mitigation. We show that a single distributional critic is sufficient to capture all uncertainty sources that contribute to overestimation: transition stochasticity, the policy-induced noise in bootstrapped targets, and learning errors, but not total return distribution in contrast to fully distributional methods \citep{bellemare2017distributional, dabney2018distributional, duan2021distributional, ma2025dsac}. By applying pessimism directly on this distribution, STAC eliminates the need for ensembles, reducing both computation and memory costs. 

Our method differs from other methods using aleatoric uncertainty for safety \citet{tang2019worst, yang2021wcsac, kim2022efficient, ma2025dsac}. These methods, incorporate total return uncertainty into policy updates, whereas STAC uses temporal (one-step) uncertainty on both critic and policy updates for overestimation mitigation rather than safe learning. However, pessimism in STAC still yields to risk-averse learning. 
...
The implementation is very simple and can be obtained by introducing dropout to networks, modeling a distributional critic and defining and pessimistic learning objective upon Soft Actor-Critic algorithm \citep{haarnoja2018soft}. We conduct experiments on standard RL benchmarks to evaluate the performance of STAC compared to existing methods. Our results demonstrate the effectiveness of STAC in achieving competitive performance to SOTA methods while requiring fewer computational resources and fewer samples, making it a promising approach for real-world RL applications. 
...
Corresponding Bellman optimality operator is
...
According to Corollary \ref{cor:pessimism}, pessimistic TD targets should be used to train critic network. However, this analysis do not account for policy learning since policy is assumed as $\text{softmax}$ over action values. In actor-crtic framework, same pessimistic objective should be used for policy improvement, since policy evaluation objective and policy improvement objective should be the same. In other words, at learning step $k$, Bellman backup must be equal to Bellman policy evaluation with the new policy, $\mathcal{T}^* Q^k \disteq \mathcal{T}^{\pi^{k+1}} Q^k$. It is only possible by using same pessimistic critic value for policy improvement. 
...
\subsection{Risk Aversion by Pessimism}

We conducted an ablation study to observe how actor behaves under stochastic environments and whether STAC yields to risk-averse learning. 
