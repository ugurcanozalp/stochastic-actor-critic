...
Recent work of \citet{nauman2024overestimation} has also shown that network regularization has positive impact on overestimation by reducing over-fitting. Ensemble based methods inherently regularized by using multiple critic networks. Since STAC uses a single critic network, it is sensitive to learning errors and over-fitting. For this purpose, dropout \citep{srivastava2014dropout} and layer normalization \citep{ba2016layer} are employed in both actor and critic networks as regularization tools.
...
\paragraph{Broader Impact} STAC is an important contribution to understand overestimation phenomenon in off-policy actor-critic learning. It demonstrates that critic overestimation is a topic of distributional learning, and the solution is to devise a risk-averse learning objective for both actor and critic. Moreover, it also proves that modeling epistemic uncertainty is not necessary at all for this problem, improving computation efficiency directly. 
