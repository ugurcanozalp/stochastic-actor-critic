...
\begin{abstract}
	...
	In this work, we propose a new algorithm called Stochastic Actor-Critic (STAC) that incorporates temporal (one-step) aleatoric uncertainty (due to noise within data) of critic to scale pessimistic bias in the temporal-difference targets, instead of epistemic uncertainty. STAC uses a single distributional critic network to model distribution in value predictions, and applies dropout to both the critic and actor networks for regularization. 
...
% Explain what is one-step uncertainty in the intro. 
% Update actor-critic update iteration notation.
...
Recently, in q-learning setting, \citet{achab2023one} proposed to model distribution of one-step return, rather than full return. This approach is proven to have convergence guarantees theoretically. Upon this idea, we hypothesize that this kind of uncertainty should be the base for scaling overestimation in actor-critic, since it includes all uncertainty sources that contribute to overestimation: transition stochasticity, the policy-induced noise in bootstrapped targets, and learning errors, but not total return distribution. 
...
The optimal policy gives the maximum value, i.e., $\pi^*(\cdot|s) = \arg\max_{\pi} V^{\pi}(s)$. Policy improvement is minimizing KL divergence from policy to softmax policy of critic, 

\begin{equation}
	\label{eqn:pi_update}
	\pi^{k+1}(\cdot \mid s) 
	= \arg\min_{\pi} \mathbb{E}_{\substack{s\sim\rho_{\pi_B}}} \Big[ \text{KL} \Big( \pi(\cdot \mid s) \Big|\Big| \frac{\exp({\alpha}^{-1} Q^{k}(s,\cdot))}{ \int_{\mathcal{A}} \exp( {\alpha}^{-1} Q^{k}(s,a)) da} \Big) \Big]
...
At each step $k$, the state-action value distribution is updated by minimizing negative log-likelihood of average distributional Bellman backup,
...
\begin{table}[h!]
	...
	\resizebox{\columnwidth}{!}{
		\begin{tabular}{|c c c c c c |}
			...
			\hline			
			\verb~Ant-v4~ & 3M & 0 & 4369.8±2289.2 & \textbf{5676.6±2032.5} & 5088.0±2295.8 \\
			\verb~BipedalWalker-v3~ & 1M & 0 & \textbf{262.7±123.1} & 245.7±150.2 & 261.5±113.3 \\
			\verb~BipedalWalkerHardcore-v3~ & 3M & 0 & 37.7±100.7 & -41.7±83.3 & \textbf{83.5±143.2} \\
			\verb~HalfCheetah-v4~ & 3M & 0 & 10320.4±2640.7 & 9363.6±2136.8 & \textbf{11291.8±3095.7} \\
			\verb~Hopper-v4~ & 1M & 0 & 1354.0±1087.9 & 1998.2±1238.5 & \textbf{2614.3±1167.4} \\
			\verb~Humanoid-v4~ & 3M & 0 & 6034.8±2738.5 & 5179.4±2161.3 & \textbf{6105.4±2739.0} \\
			\verb~LunarLander-v3~ & 1M & 0 & 154.2±166.0 & \textbf{212.6±138.2} & 201.6±135.0 \\
			\verb~Swimmer-v4~ & 1M & 0 & 76.7±26.2 & 57.3±23.2 & \textbf{97.7±32.9} \\
			\verb~Walker2d-v4~ & 1M & 0 & \textbf{4934.8±1596.1} & 3935.1±1571.9 & 4829.7±2383.8 \\		
			\hline
		\end{tabular}
	}
\end{table}
...
\begin{table}[h!]
	...
	\begin{tabular}{|c c c c c c |}
		...
		Env  & \# steps & DSAC & ESTAC & SAC & STAC \\ 
		\hline		
		\verb~Ant-v4~ & 3M & 0 & 5798.9 & \textbf{7407.1} & 6907.3 \\
		\verb~BipedalWalker-v3~ & 1M & 0 & 319.2 & \textbf{326.3} & 308.0 \\
		\verb~BipedalWalkerHardcore-v3~ & 3M & 0 & 32.3 & -50.8 & \textbf{159.9} \\
		\verb~HalfCheetah-v4~ & 3M & 0 & 13241.6 & 11475.6 & \textbf{14489.5} \\
		\verb~Hopper-v4~ & 1M & 0 & 944.3 & 3196.4 & \textbf{3363.7} \\
		\verb~Humanoid-v4~ & 3M & 0 & 8608.9 & 6990.4 & \textbf{8726.2} \\
		\verb~LunarLander-v3~ & 1M & 0 & 258.3 & \textbf{268.9} & 265.7 \\
		\verb~Swimmer-v4~ & 1M & 0 & 104.1 & 54.2 & \textbf{125.0} \\
		\verb~Walker2d-v4~ & 3M & 0 & 5952.5 & 5036.4 & \textbf{6634.7} \\
		\hline
	\end{tabular}
\end{table}
